...
for version in "${K3S_VERSIONS[@]}"; do
  ...
  k3d_test_cmd cluster create $clustername --wait --timeout 60s --image "rancher/k3s:$version" || failed "could not create cluster with k3s version $version"
  ...
  k3d_test_cmd cluster rm -a || failed "failed to delete clusters"
