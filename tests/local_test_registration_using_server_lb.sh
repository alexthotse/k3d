...
clean_up() {
  info "Cleaning up"
  k3d_test_cmd cluster delete $CLUSTER_NAME > /dev/null || true
}

check_k3s_url() {
  local node=$1
  local expected_url=$2
  local curr_url=$($RUNTIME_CMD inspect "$node" | grep K3S_URL | awk -F"=" '{print $2}' | sed 's/",//')
...
# Constants for tests
BIN=k3d
export EXE=$BIN # EXE is still used by k3d_test_cmd indirectly
...
run "k3d_test_cmd cluster create $CLUSTER_NAME -s 3"

check_multi_node $CLUSTER_NAME 3

info "Adding an agent and checking its K3S_URL"
run_with_timeout "k3d_test_cmd node create -c $CLUSTER_NAME testagent"
check_k3s_url k3d-testagent-0 $K3S_URL_DEFAULT

check_multi_node $CLUSTER_NAME 4

info "Deleting server-0"
run_with_timeout "k3d_test_cmd node delete k3d-${CLUSTER_NAME}-server-0"
run "kubectl delete node k3d-${CLUSTER_NAME}-server-0"

check_multi_node $CLUSTER_NAME 3

info "Adding a new agent to check the new K3S_URL"
run_with_timeout "k3d_test_cmd node create -c $CLUSTER_NAME testagent1"
check_k3s_url k3d-testagent1-0 $K3S_URL_LB
run_with_timeout "k3d_test_cmd node delete k3d-testagent1-0"

check_multi_node $CLUSTER_NAME 4

info "Adding a new server to check K3S_URL"
run_with_timeout "k3d_test_cmd node create -c $CLUSTER_NAME testserver --role server"
check_k3s_url k3d-testserver-0 $K3S_URL_LB
run_with_timeout "k3d_test_cmd node delete k3d-testserver-0"
...
run "k3d_test_cmd cluster create $CLUSTER_NAME -s 3 --no-lb"
